  ...
  case "$idle_type" in
    in_menu)
      case "$timeout_value" in
        Off)
		      pgrep -f 'idlemon.*mainui.py' | xargs kill -9
          return 0
          ;;
        2m)
          idle_time=120
      ...
      idlemon -p "mainui.py" -t "$idle_time" -c "$idle_count" -s "/mnt/SDCARD/spruce/scripts/idlemon_actionWrapper.sh" -i $EVENT_ARG > /dev/null &
      ;;
